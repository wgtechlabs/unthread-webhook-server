...
const parsedWebhookMaxSkewSeconds = Number.parseInt(
  process.env.WEBHOOK_MAX_SKEW_SECONDS || '300',
  10,
);
const webhookMaxSkewSeconds =
  Number.isFinite(parsedWebhookMaxSkewSeconds) &&
  parsedWebhookMaxSkewSeconds > 0
...
export const config = {
  ...
  webhookSkewEnforce:
    (process.env.WEBHOOK_SKEW_ENFORCE || 'true').toLowerCase() !== 'false',
