...
export class FileAttachmentCorrelationUtil {
  ...
  cacheMessageEvent(event: UnthreadWebhookEvent, sourcePlatform: string): void {
    ...
    if (!correlationKey || correlationKey.trim() === '') {
      LogEngine.warn(
        'Cannot cache message event - insufficient correlation data',
        {
    ...
    this.correlationTTL.set(
      correlationKey,
      Date.now() + this.FILE_ATTACHMENT_CORRELATION_TTL,
    );
    ...
    this.processBufferedFileEvent(correlationKey, sourcePlatform).catch(
      (error) => {
        LogEngine.error('Error processing buffered file event', {
          correlationKey,
          error,
        });
      },
  ...
  correlateFileEvent(event: UnthreadWebhookEvent): PlatformSource {
    ...
    if (!correlationKey || correlationKey.trim() === '') {
      LogEngine.warn(
        'Cannot correlate file attachment event - insufficient correlation data',
        {
  ...
  ): FileAttachmentCorrelationEntry | null {
  ...
  private bufferFileEvent(
    ...
    if (!correlationKey || correlationKey.trim() === '') {
      LogEngine.error(
        'Cannot buffer file event - empty correlation key would cause collisions',
        {
          fileEventId: event.eventId,
          eventData: {
            conversationId: event.data?.conversationId,
    ...
    if (existing) {
      ...
      const isDuplicate = existing.events.some(
        (e) => e.eventData.eventId === event.eventId,
      );

      if (isDuplicate) {
        LogEngine.warn(
          'Duplicate file attachment event detected, skipping buffer',
          {
        ...
        const timeoutId = setTimeout(() => {
          this.processBufferedEventsAsUnknown(correlationKey).catch((error) => {
            LogEngine.error('Error processing buffered events as unknown', {
              correlationKey,
              error,
            });
          });
      ...
      LogEngine.info('File attachment event buffered for correlation', {
        ...
        willExpireAt: new Date(
          Date.now() + this.FILE_ATTACHMENT_BUFFER_TIMEOUT,
        ).toISOString(),
      });
    }
  }
   ...
   */
  private async processBufferedFileEvent(
    correlationKey: string,
    ...
    if (!correlationKey || correlationKey.trim() === '') {
      LogEngine.warn(
        'Skipping buffered event processing - empty correlation key',
        {
          sourcePlatform,
        },
      );
    ...
    if (bufferedEvents) {
      ...
      for (const bufferedEvent of bufferedEvents.events) {
        index++;
        LogEngine.debug(
          `Processing buffered event ${index}/${bufferedEvents.events.length}`,
          {
        ...
        await this.onBufferedEventReady?.(
          bufferedEvent.eventData,
          sourcePlatform,
        );
      }
    }
   ...
   */
  private async processBufferedEventsAsUnknown(
    correlationKey: string,
  ): Promise<void> {
    ...
    if (bufferedEvents) {
      ...
      LogEngine.warn(
        'File attachment events timed out waiting for correlation',
        {
          correlationKey,
          eventCount: bufferedEvents.events.length,
          eventIds: bufferedEvents.events.map((e) => e.eventData.eventId),
      ...
      for (const bufferedEvent of bufferedEvents.events) {
        index++;
        LogEngine.debug(
          `Processing timed-out event ${index}/${bufferedEvents.events.length} as unknown`,
          {
            eventId: bufferedEvent.eventData.eventId,
            bufferedFor: Date.now() - bufferedEvent.bufferedAt,
          },
  ...
  private cleanup(): void {
    ...
    for (const [key, bufferedEvents] of this.bufferedEvents.entries()) {
      ...
      const hasStaleEvents = bufferedEvents.events.some(
        (event) =>
