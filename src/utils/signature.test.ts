...
describe('signature utils', () => {
  it('verifies unthread signature with timing-safe comparison', () => {
    ...
    expect(
      verifyUnthreadSignature(
        { ...req, headers: { 'x-unthread-signature': 'zzzz' } } as any,
        secret,
      ),
    ).toBe(false);
    expect(
      verifyUnthreadSignature(
        {
          ...req,
          headers: { 'x-unthread-signature': signature.slice(2) },
        } as any,
        secret,
      ),
    ).toBe(false);
  ...
  it('verifies legacy webhook signature with timing-safe comparison', () => {
    ...
    const signature = createHmac('sha256', secret)
      .update(rawBody)
      .digest('hex');
    const req = { headers: { 'x-signature': signature }, rawBody };

    expect(verifyWebhookSignature(req as any, secret)).toBe(true);
    expect(verifyWebhookSignature({ ...req, headers: {} } as any, secret)).toBe(
      false,
    );
    expect(
      verifyWebhookSignature(
        { ...req, headers: { 'x-signature': 'not-hex' } } as any,
        secret,
      ),
    ).toBe(false);
  });
